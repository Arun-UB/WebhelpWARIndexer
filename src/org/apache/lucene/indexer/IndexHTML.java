...
public class IndexHTML {
  ...
  public static void main(String[] argv) {
    try {
      ...
      writer = new IndexWriter(FSDirectory.open(index), new PositionalPorterStopAnalyzer(), create, 
  ...
  private static void indexDocs(File file, File index, boolean create)
    ...
    if (!create) {				  // incrementally update
      ...
      if (deleting) {				  // delete rest of stale docs
        while (uidIter.term() != null && "uid".equals(uidIter.term().field())) {
  ...
  private static void indexDocs(File file) throws Exception {
    if (file.isDirectory()) {			  // if a directory
      ...
      if (uidIter != null) {
        ...
        if (uidIter.term() != null && "uid".equals(uidIter.term().field()) &&
            uidIter.term().text().compareTo(uid) == 0) {
